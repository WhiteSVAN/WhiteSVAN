...
- [ ] **Multi-account research profile.** The dashboard already has an account selector; extend `/p/[slug]`
...
- [ ] **Redaction granularity** — today only `hideAmounts`; add hide symbols / sizes / timestamps if
      trade-level views are ever exposed on the research profile.

## Product / UX

- [ ] Back `/network` rooms and research tape with persisted posts, room membership, and moderation
...
- [ ] Add long-form stock deep dives with thesis, catalyst path, valuation notes, bear case, and
      source attachments.
- [ ] Render each published brief's **own stored metrics snapshot** in its research-profile section.
- [ ] **Email / magic-link auth** (currently credentials only; Auth.js adapter models already exist).
- [ ] **OG image + richer metadata** for shared research-profile links.
...
- Demo: `npm run db:seed` → `/p/demo` (login `demo@trustsvan.local` / `demo1234`).
